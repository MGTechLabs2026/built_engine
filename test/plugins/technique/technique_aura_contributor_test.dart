...
void main() {
  test('resolves each auraRuleId into an AuraRule with default self scope', () {
    ...
    final auras =
        TechniqueAuraContributor(_techDefWithAuras(['aura.heal']), content).auraRules();
  ...
  test('reads scope: opponent from the rule json', () {
    ...
    final auras =
        TechniqueAuraContributor(_techDefWithAuras(['aura.bleed']), content).auraRules();
    expect(auras.single.scope, AuraScope.opponent);
  });

  test('empty auraRuleIds yields no auras', () {
    expect(
      TechniqueAuraContributor(_techDefWithAuras(const []), ContentRegistry())
          .auraRules(),
      isEmpty,
    );
  });
  ...
  test('an unknown auraRuleId throws ContentNotFoundException', () {
    expect(
      () => TechniqueAuraContributor(
              _techDefWithAuras(['aura.missing']), ContentRegistry())
          .auraRules(),
  ...
  test('an invalid scope value throws ArgumentError', () {
    ...
    expect(
      () => TechniqueAuraContributor(_techDefWithAuras(['aura.bad']), content)
          .auraRules(),
