...
/// Builds the [QueryScope] a [Query]-backed [Condition] evaluates
/// against. Public (not a private `_scopeOf`) so `system_conditions.dart`'s
/// `Query`-backed conditions (`IsDiscovered`/`IsUnlocked`) can reuse it
/// too, rather than duplicating this one-liner in a second file.
QueryScope scopeOf(RuleContext context) =>
...
class HasTag implements Condition {
  ...
  bool evaluate(RuleContext context) {
    ...
    return HasTagQuery(tag).matches(subject, scopeOf(context));
  }
}
...
class HasComponent<T extends Object> implements Condition {
  ...
  bool evaluate(RuleContext context) {
    final subject = context.subject;
    if (subject == null) return false;
    return HasComponentQuery<T>().matches(subject, scopeOf(context));
  }
}
...
class ResourceAbove implements Condition {
  ...
  bool evaluate(RuleContext context) {
    ...
    return ResourceAboveQuery(resource, threshold)
        .matches(subject, scopeOf(context));
  }
}
...
class HealthBelow implements Condition {
  ...
  bool evaluate(RuleContext context) {
    final subject = context.subject;
    if (subject == null) return false;
    return HealthBelowQuery(threshold).matches(subject, scopeOf(context));
  }
}
...
class StatusActive implements Condition {
  ...
  bool evaluate(RuleContext context) {
    final subject = context.subject;
    if (subject == null) return false;
    return StatusActiveQuery(status).matches(subject, scopeOf(context));
  }
}
