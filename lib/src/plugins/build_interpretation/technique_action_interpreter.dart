...
/// Translates `technique`-typed [ResolvedBuild] components into
...
/// `EffectProfile` `active`-tier contribution under [techniqueActivePowerKey]
/// (`effectProfile().amount(EffectTier.active, techniqueActivePowerKey)`) is
/// added to the
...
class TechniqueActionInterpreter implements BuildActionInterpreter {
  ...
  CombatAction? _actionFor(
    ...
    final power =
        variant?.effectProfile().amount(EffectTier.active, techniqueActivePowerKey) ?? 0;
