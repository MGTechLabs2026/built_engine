...
| D13 | **`AffixAcquisition` is a plain engine-domain record** — `affixId`, `affixEventId`, `runId`, `runNumber`, `stat?`, `value`, `category`. It carries no `almanac.dart` type. `affix_acquisition.dart` does not import `almanac.dart`; the affix plugin has **zero** Almanac coupling. The composition boundary builds `AffixObservation` / `AffixSnapshot` from the record's fields. |
